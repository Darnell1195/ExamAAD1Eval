...
class Ex2DataRepository(
    ...
    suspend fun getAllGames(): List<Game> = remote.getGames()

    suspend fun saveAllGames(games: List<Game>) = local.saveAllGames(games)

    suspend fun getAllPlayers(): List<Player> = remote.getPlayers()

    suspend fun saveAllPlayers(players: List<Player>) = local.saveAllPlayers(players)
}